...
until oc --request-timeout=30s --kubeconfig /etc/kubernetes/static-pod-resources/kube-apiserver-certs/secrets/node-kubeconfigs/localhost.kubeconfig get node; do
  oc --request-timeout=30s --kubeconfig /var/local/csr_approver/kubeconfig get csr -o go-template="${go_template}" \
    | xargs --no-run-if-empty oc --request-timeout=30s --kubeconfig /var/local/csr_approver/kubeconfig adm certificate approve
  sleep 20
done
